...
export const newOrder = TryCatch(
  async (req: Request<{}, {}, NewOrderRequestBody>, res, next) => {
    ...
    if (
      !shippingInfo ||
      !orderItems?.length || // Ensure it's an array and not empty
      !user ||
      !user._id ||
      subtotal === undefined || // Allow 0
      tax === undefined || // Allow 0
      total === undefined // Allow 0
    ) {
      console.error("Missing Fields:", { shippingInfo, orderItems, user, subtotal, tax, total });
